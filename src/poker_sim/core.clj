...
(defn simulate
  [n known-hands community-cards]
  [community-cards known-hands n])

(defn -main 
  [& args]
  (try (let [params (load-string (str "[" (string/join args) "]"))]
         (println (apply simulate params)))
       (catch Exception e "Usage: ./core.clj <n> <known hands> <community cards>")))
